package com.threeway.jspservletcrud.dao;
...
public class LivroDao {
	...
	private static final String UPDATE_LIVRO = "UPDATE LIVRO SET TITULO = ?, AUTOR = ?, DESCRICAO = ?, PRECO = ? WHERE COD_LIVRO = ?";
	private static final String UPDATE_TITULO_AUTOR = "UPDATE LIVRO SET TITULO = ?, AUTOR = ? WHERE COD_LIVRO = ?";
	private static final String SALVAR = "INSERT INTO LIVRO(TITULO, AUTOR, DESCRICAO, PRECO) VALUES (?, ?, ?, ?)";
	private static final String CONSULTAR_TODOS = "SELECT * FROM LIVRO";
	private static final String CONSULTAR = "SELECT * FROM LIVRO WHERE TITULO LIKE ?";
	...
	public List<Livro> listarTodos() {
		...
		try(Connection con = FabricaConexao.getConexao();
				PreparedStatement banco = con.prepareStatement(CONSULTAR_TODOS)){
	...
	public void updateLivroPorId(Livro livro) {
		
		try(
				Connection con = FabricaConexao.getConexao();
				PreparedStatement consulta = con.prepareStatement(UPDATE_LIVRO)){
	...
	public List<Livro> consultar(String buscarLivro) {
		
		List<Livro> lista = new ArrayList<>();
		
		try(Connection con = FabricaConexao.getConexao();
				PreparedStatement banco = con.prepareStatement(CONSULTAR)){
			
			banco.setString(1, "%" + buscarLivro + "%");
